...
# MAGIC > (`90__pipelines/`), not the repo root — confirmed on the workspace 2026-07-05, `-e .`
# MAGIC > fails with "neither 'setup.py' nor 'pyproject.toml' found". `-e ../..` resolves to the
# MAGIC > repo root, where `pyproject.toml` actually lives.
...
# MAGIC
# MAGIC ```
# MAGIC favorites.json              edit favorites (00__config/favorites.json in the repo)
# MAGIC concept_hierarchy.json      edit concept hierarchy (00__config/ in the repo)
# MAGIC metrics_hierarchy.json      edit derived metrics hierarchy (00__config/ in the repo)
# MAGIC valuation_assumptions.json  edit valuation assumptions (00__config/ in the repo)
# MAGIC       ↓
...
# MAGIC ```
# MAGIC
# MAGIC > **Note:** `02__tickers_master`, `03__concept_hierarchy_master` and `04__metrics_hierarchy_master` live in `00__config/`.
...
# MAGIC
# MAGIC Assumptions (WACC, growth, etc.) live in `00__config/valuation_assumptions.json`
...
# MAGIC per-notebook timeout (same fix as `50__publish/53__republish`). If export or
...
# MAGIC Databricks secret — see `50__publish/README.md`.
...
# MAGIC snapshot so the run history is queryable over time (read with `30__analysis/36__run_log_report`).
