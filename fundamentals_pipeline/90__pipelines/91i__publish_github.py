...
# MAGIC %md
# MAGIC # 90__pipelines / 91i__publish_github
# MAGIC
# MAGIC **Part of the "Financial Analysis Pipeline" multi-task split, Phase 2** — runs
# MAGIC `52__publish_to_github` as its own Databricks Job Task. Reads the artifacts from the
# MAGIC `main.financials._publish` Unity Catalog Volume (written by `91h__export_dashboard_data`
# MAGIC in the previous task), not from driver-local `/tmp` — this task runs on its own separate
# MAGIC driver.
...
run_id = dbutils.jobs.taskValues.get(
    taskKey="export_dashboard_data", key="run_id",
    debugValue=datetime.utcnow().strftime("%Y%m%dT%H%M%SZ"),
)
pipeline_start = datetime.strptime(run_id, "%Y%m%dT%H%M%SZ")
try:
    dbutils.jobs.taskValues.set(key="run_id", value=run_id)  # relay for 91j
except Exception:
    pass
print(f"✓ run_id={run_id} (relayed from export_dashboard_data)")

# COMMAND ----------

# MAGIC %md
# MAGIC ## Publish to GitHub Release
# MAGIC Requires the `github/github_pat` Databricks secret — see `50__publish/README.md`.

# COMMAND ----------

print("=" * 55)
print("Publish to GitHub")
print("=" * 55)
_t0 = time.monotonic()
...
try:
    ...
    if STEP_TIMINGS:
        ...
        spark.createDataFrame(_runs_records, schema=_runs_schema) \
            .createOrReplaceTempView("incoming_pipeline_runs_publish")
        spark.sql(f"""
            MERGE INTO {_runs_tbl} AS t
            USING incoming_pipeline_runs_publish AS s
...
print("✓ 91i (publish_github) complete — the public GitHub Release is current.")
