...
# MAGIC %md
# MAGIC # 90_pipelines / 92__reconciliation_job — filing reconciliation (Tier A + Tier B/C)
# MAGIC
...
# MAGIC
# MAGIC 1. `10_ingestion/14__fetch_oracle_statements` — builds the linkbase oracle
# MAGIC    (`main.config.reconciliation_oracle`) for the golden set ∪ `COMBINED_FILERS`.
# MAGIC 2. `30_analysis/35__reconcile_filings` — Tier B/C (universe-wide) + Tier A (golden set)
...
_t0 = time.monotonic()
try:
    dbutils.notebook.run("../10_ingestion/14__fetch_oracle_statements", timeout_seconds=ORACLE_TIMEOUT)
...
_t0 = time.monotonic()
try:
    dbutils.notebook.run("../30_analysis/35__reconcile_filings", timeout_seconds=RECON_TIMEOUT)
