# 60__frontends/61__streamlit — public financials dashboard
...
Databricks pipeline (nightly)
    └── 50__publish/51__ → writes 5 artifacts to /tmp/ (data, metrics, prices, backtest, meta)
    └── 50__publish/52__ → uploads to GitHub Release (tag `latest`)
                              │
                              ▼
...
> shared pure module `fundamentals_pipeline/backtest.py` — the same code the pipeline
> notebook uses, so the app and the pipeline reconcile by construction. `fundamentals_pipeline`'s
> importable modules carry no Databricks dependency, so the app stays credential-free.
