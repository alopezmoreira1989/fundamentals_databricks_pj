"""Fetch and cache the parquet artifacts published by 50__publish/."""
...
def _render_load_error(exc: Exception) -> NoReturn:
    ...
    if isinstance(exc, SchemaError):
        st.error(
            ...
            "or re-run the publish step (`50__publish/51` → `52`).\n\n"
            f"```\n{exc}\n```"
        )
        st.stop()
    is_404 = (
    ...
    if is_404:
        st.error(
            ...
            "1. `50__publish/51__export_dashboard_data`\n"
            "2. `50__publish/52__publish_to_github`\n\n"
