...
``has_logo`` (from the meta artifact) suppresses a known miss up front; the ``<img onerror>``
still swaps to the monogram if the CDN 404s at render time. No bytes are cached — it's a runtime
hotlink, same as the Streamlit dashboard.
"""
...
@register.inclusion_tag("companies/_logo.html")
