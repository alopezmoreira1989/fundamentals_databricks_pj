"""Favorites views — all login-required. The page lists the user's favorite tickers (with
company names); add/remove are POST-only and redirect back to a safe ``next`` (default: the page).
"""
...
def favorites_page(request: HttpRequest) -> HttpResponse:
    """The user's favorites: each ticker with its company summary (if known)."""
...
def add(request: HttpRequest) -> HttpResponse:
    ...
    return redirect(_safe_next(request, reverse("favorites:list")))
