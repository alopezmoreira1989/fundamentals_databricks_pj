...
| `ORACLE_VALUE_MISMATCH` | app has a value, but it disagrees with the **highest-priority presented** us-gaap tag (wrong synonym picked, or value mangled) — invisible to Tier B/C, which matches the app's own chosen raw fact. (Two Cash-Flow concepts are excluded: **Net Income** — the IS covers its value; the CF line legitimately uses total `ProfitLoss` incl. NCI — and **Sales of Investments** — a sales-vs-maturities sibling-line ambiguity where the oracle's magnitude tie-break picks the larger maturities line; validator noise, not a value mangle.) | `synonym+priority` — `CONCEPT_PRIORITY` / inspect `21__clean_and_merge` → [[xbrl-concept-mapping]] |
...
| **Short-term Debt** [Balance Sheet] | `ORACLE_VALUE_MISMATCH` | **suppressed** (carve-out in `35`) | Unlike Sales of Investments, summing here was the **correct fix** (PR #88, 2026-06-26). `DebtCurrent` is the us-gaap aggregate of two disjoint components — `ShortTermBorrowings` (commercial paper / revolver) + `LongTermDebtCurrent` (current maturities). CP issuers (LIN, WMT, many large-caps) present only the parts; ingestion now SUMS them (`extract_series_aggregate_or_sum` / `AGGREGATE_OR_SUM_CONCEPTS` in `11`), un-doing a leverage undercount (LIN FY24 2.057B→6.280B). The Tier-A oracle is single-line, so the app (the sum) **always** exceeds the oracle winner (the larger component) — structural noise, not a mangle. Tier B/C still guards the value (raw now stores the summed fact). |
